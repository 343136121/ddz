import { _decorator, Prefab, isValid, instantiate, macro, Canvas, resources, find } from 'cc';
...
export default class PopupManager {
    ...
    public static show<Options>(path: string, options?: Options, params?: PopupParamsType): Promise<ShowResultType> {
        return new Promise(async res => {
            ...
            if (!isValid(node)) {
                ...
                // 加载失败（一般是路径错误导致的）
                if (!isValid(prefab)) {
                    console.warn('[PopupManager]', '弹窗加载失败', path);
                    this._current = null;
            ...
            if (!popup) {
                console.warn('[PopupManager]', '未找到弹窗组件', path);
                this._current = null;
                res(ShowResultType.Failed);
                return;
            }
            ...
            popup.finishCallback = async (suspended: boolean) => {
                ...
                await new Promise(_res => {
                    let c:Canvas = find('Canvas').getComponent(Canvas)
                    c.scheduleOnce(_res, this.interval);
                });
                // 下一个弹窗
                this.next();
            }
     ...
     * @param path 弹窗路径
     */
    private static getNodeFromCache(path: string): any {
        // 从节点缓存中获取
    ...
    private static parseParams(params: PopupParamsType) {
        ...
        if (Object.prototype.toString.call(params) !== '[object Object]') {
            console.warn('[PopupManager]', '弹窗参数无效，使用默认参数');
            return new PopupParamsType();
        }
        // 缓存模式
